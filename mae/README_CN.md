...
├── model_zoo
    ├── mae
        ...
        │        ├──mixup.py                  // 自定义数据强增策略
        │        ├──monitor.py                // 模型状态监测函数   
        │        ├──moxing_adapter.py         // 读取modelarts上信息并传递数据        
        │        ├──trainer.py                // 训练过程定义    
        │    ├── eval.py                      // 评估脚本  
        │    ├── finetune.py                  // 微调训练脚本        
        │    ├── pretrain.py                  // 预训练脚本
        ├── mindspore_mae.ipynb               // mae模型预训练jupyter案例                      
        ├── README_CN.md                      // mae模型相关说明
        ├── weight_pytorch2mindspore.md       // 预训练模型权重转换代码
...
- GPU处理器环境运行
  ...
  为了保证分布式训练的正常进行，需要先对分布式环境进行配置。
  
  - OpenMPI-4.0.3：MindSpore采用的多进程通信库。
  
    参考OpenMPI官网教程安装：https://www.open-mpi.org/faq/?category=building#easy-build。
    
  - 若涉及多机训练，则需要配置多机间免密登陆，可参考以下步骤进行配置：
    
    1. 每台主机确定同一个用户作为登陆用户
    
    2. 执行ssh-keygen -t rsa -P ""生成密钥
    
    3. 执行ssh-copy-id DEVICE-IP设置需要免密登陆的机器IP
    
    4. 执行ssh DEVICE-IP，若不需要输入密码即可登录，则说明以上配置成功
...
- 在Ascend环境运行时评估ImageNet数据集
  ...
  ```
## 预训练模型转换
将预训练模型从Pytorch迁移到MindSpore上的方法具体写在了weight_pytorch2mindspore.ipynb文件中，其中需要修改的参数只有输入的Pytorch模型路径以及输出的MindSpore模型路径位置。
